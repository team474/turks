...
function setCssNumberVar(targets: gsap.TweenTarget, varName: string, value: number) {
  ...
  (vars as Record<string, number>)[varName] = value;
...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    if (itemEls.length) {
      ...
      if (numberEls.length) {
        tl.to(
          numberEls,
          { duration: 0.6, ease: 'power2.out', stagger: { each: 0.08, from: 'start' } },
          itemsStart + 0.1
        );
        // After the stagger, ensure the CSS var is fully visible
        tl.set(numberEls, {}, itemsStart + 0.1);
        setCssNumberVar(numberEls, '--sm-num-opacity', 1);
      }
    }
  ...
  return (
    <div
      className={`sm-scope ${isFixed ? 'fixed top-0 left-0 w-screen h-screen overflow-hidden z-40' : 'w-full h-full'}`}
    >
      <div
        className={(className ? className + ' ' : '') + 'staggered-menu-wrapper relative w-full h-full z-40'}
        style={accentColor ? ({ ['--sm-accent']: accentColor } as CSSVarProperties) : undefined}
      ...
      >
        ...
        {renderInlineToggle && (
          <button
            ref={toggleBtnRef}
            className={`sm-toggle inline-flex items-center gap-[0.3rem] bg-transparent border-0 cursor-pointer font-medium leading-none overflow-visible ${
              open ? 'text-black' : 'text-[#1D431D]'
            }`}
            aria-label={open ? 'Close menu' : 'Open menu'}
            aria-expanded={open}
            aria-controls="staggered-menu-panel"
        ...
        >
          <div className="sm-panel-inner flex-1 flex flex-col items-end text-right gap-5">
            <button
              ...
              className="ml-auto -mt-10 text-[#1D431D] text-6xl font-bold leading-none"
            >
              ...
              {items && items.length ? (
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                    <Link
                      className="sm-panel-item relative text-[#1D431D] font-semibold text-[4rem] cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]"
                      href={it.link}
                ...
                <li className="sm-panel-itemWrap relative overflow-hidden leading-none" aria-hidden="true">
                  <span className="sm-panel-item relative text-[#1D431D] font-semibold text-[4rem] cursor-pointer leading-none tracking-[-2px] uppercase transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]">
                    <span className="sm-panel-itemLabel inline-block [transform-origin:50%_100%] will-change-transform">
                      No items
                    </span>
                  </span>
                  ...
                  {socialItems.map((s, i) => (
                    <li key={s.label + i} className="sm-socials-item">
                      <a
                        ...
                        className="sm-socials-link text-[1.2rem] font-medium text-[#1D431D] no-underline relative inline-block py-[2px] transition-[color,opacity] duration-300 ease-linear"
                      >
                        {s.label}
                      </a>
                    </li>
...
.sm-scope .sm-overlay { position: fixed; inset: 0; z-index: 8; background: rgba(0,0,0,0.55); opacity: 0; pointer-events: none; transition: opacity 200ms ease; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); }
.sm-scope .sm-overlay[data-open] { opacity: 0.05; pointer-events: auto; }
...
.sm-scope .staggered-menu-panel { position: fixed; top: 0; right: 0; width: clamp(260px, 38vw, 420px); height: 100%; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); display: flex; flex-direction: column; padding: 6em 2em 2em 2em; overflow-y: auto; z-index: 10; }
...
.sm-scope .sm-socials-link { font-size: 1.2rem; font-weight: 500; color: #1D431D; text-decoration: none; position: relative; padding: 2px 0; display: inline-block; transition: color 0.3s ease, opacity 0.3s ease; }
.sm-scope .sm-socials-link:hover { color: var(--sm-accent, #ff0000); }
.sm-scope .sm-panel-title { margin: 0; font-size: 1rem; font-weight: 600; color: #fff; text-transform: uppercase; }
.sm-scope .sm-panel-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0.5rem; }
.sm-scope .sm-panel-item { position: relative; color: #1D431D; font-weight: 600; font-size: 4rem; cursor: pointer; line-height: 1; letter-spacing: -2px; text-transform: uppercase; transition: background 0.25s, color 0.25s; display: inline-block; text-decoration: none; padding-right: 1.4em; }
...
@media (max-width: 1024px) { .sm-scope .sm-panel-inner { margin-left: auto; max-width: 90vw; } }
@media (max-width: 640px) { .sm-scope .staggered-menu-panel { width: 100%; left: 0; right: 0; } .sm-scope .staggered-menu-wrapper[data-open] .sm-logo-img { filter: invert(100%); } .sm-scope .sm-panel-inner { margin-left: auto; max-width: 86vw; padding-right: 1rem; } .sm-scope .sm-panel-item { font-size: 2.6rem; letter-spacing: -1px; } }
      `}</style>
    </div>
  );
};
