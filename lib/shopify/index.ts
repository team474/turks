...
import {
  ...
  ShopifyMetaObjectOperationByType,
...
export async function getMetaObjectsByType(
  type: string,
  first: number = 250
): Promise<MetaObject[]> {
  "use cache";
  ...
  try {
    const res = await shopifyFetch<ShopifyMetaObjectOperationByType>({
      query: getMetaObjectQueryByType,
      variables: {
        type,
        first,
      },
    ...
    const metaobjects = res.body.data.metaobjects.edges.map((edge) => edge.node);
    
    return metaobjects;
  } catch (error) {
    console.error('Error fetching metaobjects by type:', error);
    return [];
  }
}
...
export async function getBlogArticle(handle: string): Promise<BlogArticle | undefined> {
  ...
  try {
    // Fetch all articles from the "flower" blog and find the one with matching handle
    ...
    if (res.body.data.blogByHandle?.articles) {
      const articles = removeEdgesAndNodes(res.body.data.blogByHandle.articles);
...
export async function getBlogArticlesByBlog(
  ...
  const res = await shopifyFetch<ShopifyBlogArticlesByBlogOperation>({
    query: getBlogArticlesByBlogQuery,
    variables: {
      blogHandle,
      first,
    },
